...
#   Updated: 2018-03-23 - minor updates for v3.4.4; added detection of run directory
#
##########################################################################
...
yget(){
	...
	#echo "yget: $src --> $dst"
	if [ -n "$bHasCurl" ] ; then
		#"echo "curl -sk --max-time 15 -o "$dst" --header "Pragma: no-cache" --header "Cache-Control: no-cache" -A YAMon-Setup "$src""
...
[ -n "$(which clear)" ] && bCanClear=1
[ -n "$(which curl)" ] && bHasCurl=1
...
**************************************
   ...
   For installation tips & tricks, see
      http://usage-monitoring.com/install
